...
WalletModel::SendCoinsReturn WalletModel::sendCoinsAnon(const QList<SendCoinsRecipient> &recipients, const CCoinControl *coinControl)
{
...

    {
        ...
        } else
        {
            ...
            std::string sError;
            LogPrintf("wtxNew.nTime: %d, Params().RSABTime(): %d, wtxNew.nTime >= Params().RSABTime(): %d, RS: %d\n", wtxNew.nTime,Params().RSABTime(), wtxNew.nTime >= Params().RSABTime(),wtxNew.nTime >= Params().RSABTime() ? RING_SIG_2 : RING_SIG_1);
